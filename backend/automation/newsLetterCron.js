    ...
    for (const job of jobs) {
      try {
        ...
        for (const user of filteredUsers) {
          ...
          const message = `Hi ${user.name},\n\nGreat news! A new job that fits your rolehas just been posted. The position is for a ${job.title} with ${job.companyName}, and they are looking to hire immediately.\n\nJob Details:\n- **Position:** ${job.title}\n- **Company:** ${job.companyName}\n- **Location:** ${job.location}\n- **Salary:** ${job.salary}\n\nDon’t wait too long! Job openings like these are filled quickly. \n\nWe’re here to support you in your job search. Best of luck!\n\nBest Regards,\ncompany Team`;
          sendEmail({
            email: user.email,
            subject,
            message,
